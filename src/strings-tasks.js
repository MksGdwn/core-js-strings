...
function orderAlphabetically(str) {
  const arr = str.split('');
  for (let i = 0; i < str.length; i += 1) {
    for (let j = 0; j < str.length - i - 1; j += 1) {
...
function isPalindrome(str) {
  const newStr = str.toLocaleLowerCase().replace(/[!?, .]/g, '');
...
function reverseWords(str) {
  const arr = str.split(' ').map((el) => el.split('').reverse().join(''));
  return arr.join(' ');
}
...
function getCardId(value) {
  ...
  const newValue = value.replace(/[AJQK]/, (char) => data[char]);
  const result = +data[newValue.slice(-1)] + +[newValue.slice(0, -1)] - 1;
  return result;
}
